...
void calculateTargets(unsigned long time) {
  switch (currentState) {
    ...
    case MODE_6_ROLL: {
      // Continuous rolling wave moving to one side using a triangle wave (no sine slowdown/pause)
      float period = 2400.0; // 2.4 second cycle
      float phaseDelay = 400.0; // 400ms delay between adjacent wings
      
      for (int i = 0; i < 4; i++) {
        // Shift time for each wing to create a wave propagation to one side (0 -> 1 -> 2 -> 3)
        float tShifted = time - (i * phaseDelay);
        
        // Calculate triangle wave between 0.0 and 1.0
        float tNormalized = fmod(tShifted, period);
        if (tNormalized < 0) tNormalized += period; // Handle negative fmod
        tNormalized /= period;
        
        float triVal = 0.0;
        if (tNormalized < 0.5) {
          triVal = tNormalized * 2.0; // Rise from 0.0 to 1.0
        } else {
          triVal = 2.0 - (tNormalized * 2.0); // Fall from 1.0 to 0.0
        }
        
        // Map to a smooth, calming range (60 to 120 degrees)
        targetPos[i] = 60.0 + (triVal * 60.0);
      }
      break;
    }
